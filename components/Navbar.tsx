...
export default function Navbar() {
  ...
  return (
    <>
          ...
          >
            ...
            <div className="relative h-full flex flex-col items-center justify-center gap-8">
              {navLinks.map(({ label, href }, index) => (
                ...
                >
                  <span className="text-nexus-accent font-mono text-sm mr-2">
                    0{index + 1}.
                  </span>
                  {label}
                </motion.button>
              ...
              {/* Decorative */}
              <motion.div
                className="absolute bottom-12 left-0 right-0 flex justify-center gap-4"
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                transition={{ delay: 0.5 }}
              >
                <span className="text-xs font-mono text-nexus-muted/40">
      ...
      <div className="hidden lg:flex fixed right-[72px] top-1/2 -translate-y-1/2 z-[9996] flex-col gap-3">
        {navLinks.map(({ label, href }) => (
          <motion.button
            key={href}
            onClick={() => handleNavClick(href)}
