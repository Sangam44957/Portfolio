        ...
        <>
          <div className="grid grid-cols-3 gap-4 mb-6">
            {[
              ...
              {
                label: "Avg/Day",
                value: (totalContributions / 365).toFixed(1),
              },
            ].map((stat) => (
              <div
                key={stat.label}
                className="bg-nexus-surface/50 rounded-lg p-3 text-center"
              >
              ...
              {weeks.map((week, weekIndex) => (
                <div key={weekIndex} className="flex flex-col gap-[3px]">
                  {week.map((day, dayIndex) => (
                    <motion.div
                      key={day.date}
                      className={`w-[13px] h-[13px] rounded-[3px] cursor-pointer transition-all duration-150 ${
                        levelColors[day.level]
                      } ${levelHoverColors[day.level]} hover:ring-1 hover:ring-nexus-accent/40`}
          ...
          <div className="flex items-center gap-2 mt-4 justify-end">
            <span className="text-[10px] text-nexus-muted/40 font-mono">
              Less
            </span>
            {[0, 1, 2, 3, 4].map((level) => (
              <div
                key={level}
                className={`w-[13px] h-[13px] rounded-[3px] ${levelColors[level]}`}
              />
            ))}
            <span className="text-[10px] text-nexus-muted/40 font-mono">
              More
