...
function TimelineItem({
  ...
  const typeConfig = {
    work: {
      icon: FiBriefcase,
      color: "#00f0ff",
      gradient: "from-nexus-accent/20 to-nexus-accent/5",
    },
    education: {
      icon: FiBookOpen,
      color: "#7b61ff",
      gradient: "from-nexus-accentAlt/20 to-nexus-accentAlt/5",
    },
    achievement: {
      icon: FiAward,
      color: "#ff006e",
      gradient: "from-nexus-pink/20 to-nexus-pink/5",
    },
  };
    ...
    >
      ...
      >
        ...
        >
          ...
          {/* Description */}
          <ul
            className={`space-y-2 mb-5 ${isEven ? "" : "md:text-right"}`}
          >
          ...
          >
            {experience.techUsed.map((tech) => (
              <span
                key={tech}
                className="px-2 py-1 rounded-md text-xs font-mono bg-nexus-surface text-nexus-muted border border-nexus-border/50"
              >
                {tech}
              </span>
            ))}
...
export default function Experience() {
  ...
  return (
    <SectionWrapper
      id="experience"
      title="Journey"
      subtitle="My professional path and the milestones along the way"
      number="04"
    >
        ...
        <div className="absolute left-5 md:left-1/2 md:-translate-x-[0.5px] top-0 bottom-0 w-[1px] bg-nexus-border/20">
          <motion.div
            ...
            style={{
              height: lineHeight,
              background:
                "linear-gradient(180deg, #00f0ff, #7b61ff, #ff006e, #00ff88)",
            }}
          />
        </div>
