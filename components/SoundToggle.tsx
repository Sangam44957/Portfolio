...
export default function SoundToggle({ isMuted, toggleMute }: SoundToggleProps) {
  const handleToggle = () => {
    toggleMute();
    // Play a sound when toggling (if unmuting)
    if (isMuted) {
  ...
  return (
    <motion.button
      ...
      className="fixed right-6 bottom-6 z-[9995] w-12 h-12 rounded-full flex items-center justify-center overflow-hidden group glass"
    ...
    >
      ...
      {!isMuted && (
        <>
          <motion.div
            className="absolute inset-0 rounded-full border border-nexus-accent/20"
            animate={{ scale: [1, 1.6], opacity: [0.3, 0] }}
            transition={{ duration: 2, repeat: Infinity }}
          />
          <motion.div
            className="absolute inset-0 rounded-full border border-nexus-accent/10"
